...
class WaitingListBase(models.Model):
    ...
    def _search_customer_phone(self, operator, value):
        return [('customer_id.phone', operator, value)]
    
    @api.depends('customer_id')
    def _compute_customer_mobile(self):
        for record in self:
            # In Odoo 19, res.partner uses 'phone' field for mobile phone
            if record.customer_id:
                record.customer_mobile = record.customer_id.phone or False
            elif not record.customer_mobile:
                record.customer_mobile = False
        ...
        if self.customer_mobile:
            ...
            if core_number and len(core_number) >= 9:
                ...
                if len(pattern_conditions) == 1:
                    # Only one pattern: simple condition
                    domain = ['&', pattern_conditions[0], ('is_company', '=', False)]
                else:
                ...
                if partner:
                    ...
                    _logger.info(f'Found existing customer: {partner.name} (ID: {partner.id}, phone: {partner.phone})')
            ...
            if not vals.get('customer_id'):
                ...
                if has_mobile and has_name:
                    ...
                    customer_vals = {
                        ...
                        'phone': vals.get('customer_mobile') or vals.get('customer_phone'),
            ...
            if not vals.get('customer_id') and not record.customer_id:
                if vals.get('customer_mobile') or (record.customer_mobile and 'customer_name' in vals):
                    customer_vals = {
                        ...
                        'phone': vals.get('customer_mobile', record.customer_mobile) or vals.get('customer_phone', record.customer_phone),
    ...
    def action_mark_seated(self):
        ...
        for record in self:
            ...
            if record.waiting_type == 'waitlist' and hasattr(record, 'table_id') and not record.table_id:
